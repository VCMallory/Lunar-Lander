...
void activegame4(ALLEGRO_EVENT_QUEUE** queue, ALLEGRO_EVENT* event, int* onEmpezar, ALLEGRO_BITMAP** nave, ALLEGRO_BITMAP** empezarbackground, ALLEGRO_KEYBOARD_STATE* keystate, float* fuel, int * score)
{
    ...
    plataforma1 = al_load_bitmap("resources/background/plataforma4.png");
    ...
    *onEmpezar = 1;
    ...
    while (*onEmpezar == 1) {
        ...
        if (y > 402 && x > 112 && x < 173 && v < 0.6) {
            *score = *score + 100;
            *onEmpezar = 0;/*WINS*/
            activegame5(&*queue, &*event, &*onEmpezar, &*nave, &*empezarbackground, &*keystate, &*fuel, &*score);
        }
        ...
        if (event->type == ALLEGRO_EVENT_TIMER) {
            ...
            al_draw_textf(displayFuel, al_map_rgb(255, 255, 255), 780, 20, ALLEGRO_ALIGN_RIGHT, "Press P to pause");
            al_draw_bitmap(plataforma1, 0, 240, 0); /*PLATFORM 1*/
